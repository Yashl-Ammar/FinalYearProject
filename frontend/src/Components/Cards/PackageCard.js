...
function PackageCard({disabled,title,description, registerPrice, registerTime, registerOffer, registerRevision, bullet, setBullet, allBullet, setAllBullets, bulletsError, errorPrice, errorTime, errorOffer, errorRevision, priceValue, timeValue, offerValue, revisionValue}) {
    ...
    return ( 
    <div className="bg-aamdanSuperDeepWhite dark:bg-aamdanSuperDeepBlack rounded-xl w-full px-8 py-8">
        <h2 className="text-3xl font-bold mb-8">{title}</h2>
        <p className="text-lightGrayWhite dark:text-lightGray mb-8">{description}</p>
        <div className="mb-8">
        ...
        <div className="flex items-center justify-between w-full mb-4">
            ...
            <p className={`${bullet?.length > 30 ? 'text-red' : 'text-lightGrayWhite dark:text-lightGray ' }`}>{bullet ? bullet.length : 0}/30</p>
        </div>
        <div className="w-full flex flex-col lg:flex-row mb-6">
            <input className="rounded-lg bg-white dark:bg-aamdanBackground py-3 px-5 border border-strokeColor border-opacity-50 w-full" placeholder='e.g 5 screen' value={bullet} onChange={(e) => {setBullet(e.target.value)}} disabled={disabled} />
        ...
        <p className="text-lightGrayWhite dark:text-lightGray mb-6">Description in Bullets</p>
